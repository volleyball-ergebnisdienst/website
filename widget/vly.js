...
(function (window, document, undefined) {
    ...
    // TODO why global contex vly?
    window.vly.createCallbackTag = function (container) {
    ...
    window.vly.createGamesCallbackTag = function (container) {
        var gamesSrc = document.createElement('script');
        var gamesName = createGamesCallback(container);
        if (container.hasAttribute('data-team')) {
            gamesSrc.src = 'https://volleyball-ergebnisdienst.de/api/schedules/' + container.dataset.leagueId + '?team=' + container.dataset.team + '&callback=' + gamesName;
        } else {
    ...
    var logRequests = function () {
        ...
        xhr.open("POST", "https://volleyball-ergebnisdienst.de/api/logs", true);
    ...
    // only call once
    if (window.vly.called) {
        console.error("Vly function already called. Aborting.");
        return;
    }
    // default styling
    ...
    for (var i = 0; i < containers.length; i += 1) {
        if (containers[i].hasAttribute('data-games')) {
            window.vly.createGamesCallbackTag(containers[i]);
        } else {
            window.vly.createCallbackTag(containers[i]);
        }
    }
    logRequests();
    window.vly.called = true;
}(window, document));
